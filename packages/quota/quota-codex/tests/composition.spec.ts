 ...
 *
 * `reading.spec.ts` covers the reader in isolation, so nothing here was under
 * test before: the plugin's refresh path had no coverage at all. The case that
 * matters is a refresh requested while a read is still in flight, which is what
 * a click during the boot read is. The first read is therefore held open on
 * purpose rather than being allowed to settle first — letting it settle is the
 * easy path, and it passes whether or not the request is honoured.
 */
...
vi.mock('../src/reading.ts', () => ({ readQuota: vi.fn(), codexBinary: () => 'codex' }))

const rows = [{ id: 'weekly', label: 'Weekly', primary: { remaining: 89.4, minutes: 10_080, resetsAt: null }, secondary: null }]
...
async function writeConfig(root: string): Promise<void> {
  ...
  // `baseUrl`, so a bare `settings.yaml` here would be written into the
  ...
  await writeFile(
    ...
    [
      ...
      "- name: '@deepseek-ai/dsh-quota-codex'",
      '  config:',
      '    refreshIntervalMs: 0',
      '',
    ].join('\n'),
...
async function boot(root: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-quota-codex', quota],
  ])
...
it('boots the YAML reader, publishes quota and disposes its namespace', async () => {
  const root = await mkdtemp(join(tmpdir(), 'codex-composition-'))
  vi.mocked(readQuota).mockReset()
  vi.mocked(readQuota).mockResolvedValue(rows)
  const context = await boot(root)
  try {
    const snapshot = () => context.settings.describe().find(row => row.ns === quota.CODEX_QUOTA_NAMESPACE)
    ...
    await context.settings.update(quota.CODEX_QUOTA_NAMESPACE, { refreshRequestedAt: 1 })
    ...
    const entry = [...context.loader.entries()].find(row => row.options.name === '@deepseek-ai/dsh-quota-codex')
...
it('serves a refresh requested while the boot read is still in flight', async () => {
  const root = await mkdtemp(join(tmpdir(), 'codex-refresh-'))

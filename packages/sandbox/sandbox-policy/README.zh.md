...
`requireWriteConfirmation` 要求用户在会话权限控件选择 workspace-write，随后在同一会话发送完整 go；单独批准、单独 go 或逐文件提权不能绕过。委派代理不能自批；切回只读、释放或重启撤销授权。`confinedOnly` 拒绝完全访问提权，将已保存的不受限模式解析为只读。基础组合启用两项设置，本机 CLI 权限仍独立。`writeApprovalTtlMs` 指定待确认批准有效期，默认 900,000 毫秒；确认后的授权持续到撤销或策略实例结束。

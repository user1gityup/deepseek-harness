...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/quota/quota-antigravity': { kind: 'none', reason: 'Local quota reader publishes settings without model context.' },
  'packages/client/ui-antigravity-quota': { kind: 'none', reason: 'Sidebar projection contributes no model context.' },
  ...
  'packages/client/ui-claude-quota': { kind: 'none', reason: 'Browser-side sidebar panel over one settings namespace; it registers no tool, prompt, or model-visible text.' },
  'packages/quota/quota-claude': { kind: 'none', reason: 'Host reader publishing CLI-reported quota into one settings namespace; nothing it writes reaches a model request.' },

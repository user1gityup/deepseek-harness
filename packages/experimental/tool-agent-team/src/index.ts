...
function install(agent: Agent, ctx: Context, config: Required<Config>): () => void {
  ...
  try {
    ...
    register(scoped.tools.register(defineTool({
      name: 'list_providers',
      description:
        'List the subagent providers a teammate can be spawned on. Call this before '
        + 'spawn_teammate when choosing a provider, so the name is one that exists.',
      parameters: {},
      output: {
        schema: {
          type: 'object',
        ...
        render: (_args: unknown, value: { providers: readonly string[]; default: string }) => [{
          type: 'text' as const,
          text: value.providers.length === 0
            ? 'no subagent providers are registered'
    ...
    register(scoped.tools.register(defineTool({
      ...
      description:
        ...
        + 'Teammates may run on different providers: pick the one whose strengths match the '
        + 'delegated work rather than spawning every teammate on the default.',
      parameters: {
        ...
        provider: {
          type: 'string',
          description:
            'Subagent provider to run this teammate on. Omit to use the configured default. '
            + 'Call list_providers first to see what is registered — an unknown name is rejected.',
        },
      },
      ...
      async execute(args, exec) {
        ...
        // A per-teammate provider is what makes a team heterogeneous: the
        // roster already stores `provider` per member and descriptor matching
        // already keys off it, so only the choice was missing.
        const fallback = context === 'fork' ? config.forkProvider : config.freshProvider
        const provider = args.provider ?? fallback
        if (args.provider !== undefined) {
          // Fail here with the list rather than deep inside dispatch: a bad
          // provider name is a recoverable authoring mistake, and the model
          // can only correct it if it is told what the valid names are.
          const known = ctx.subagents.list()
          if (!known.includes(args.provider)) {
            throw new Error(
              `unknown subagent provider "${args.provider}" — registered providers: ${known.join(', ') || '(none)'}`,
            )
          }
        }
